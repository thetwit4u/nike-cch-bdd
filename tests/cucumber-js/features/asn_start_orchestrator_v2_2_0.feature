Feature: Controller ASN start → Orchestrator HITL flow v2.2.0 (mocked)
  ...
  Scenario: Flow Controller starts orchestrator from ASN; validate deliveries only
    ...
    # Optional: assert correlation contains the injected UUID on the last received event
    And the last System Event jsonpath "$.correlationId" contains "${ctx:captures.controllerStartUuid}"
